...
export default function Lista (props){
 const { lista } = props
 const {infoTransporte, setInfoTransporte, setStatus} = useContext(GlobalContext)
 ...
 const status =(item)=>{
    if(item.inicioSeparacao === null ){
        ...
        return {status:"Aguardando Conferencia",texto:"Desejar iniciar Conferencia?"}
    }
    else if(item.fimCarregamento === null){

        return {status:"Em carregamento",texto:"Deseja finalizar Carregamento?"}
    } else {
 ...
 const navigate = useNavigate();
    ...
    return <div style={{marginTop:15}}>{lista?.map(item=> <div onClick={()=>irParaDetalhes(item, status(item).texto)} style={{backgroundColor: "#92b0e7", margin:4, borderRadius:10, padding:5}}> 
            <div>{item.NRota}</div>
